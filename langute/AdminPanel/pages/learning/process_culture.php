...
if (empty($title) || empty($created_at) || empty($content) || empty($_FILES["image"]["name"])) {
    ...
    echo "<script>window.location.href = 'addculture.php';</script>";
} else {
    // تبدیل تاریخ انتخاب شده به فرمت مناسب برای ذخیره در دیتابیس
    $created_at = date("Y-m-d H:i:s", strtotime($created_at));
// محدودیت‌های مربوط به عکس
$serverAddress = $_SERVER['SERVER_ADDR'];
$src="../../../images/uploads/culture/" . basename($_FILES["image"]["name"]);
$targetDir = "/langute/Project-files/langute/images/uploads/culture/";
...
elseif ($_FILES["image"]["size"] > $maxFileSize) {
    ...
    echo "<script>window.location.href = 'addculture.php';</script>";
      ...
      elseif (move_uploaded_file($_FILES["image"]["tmp_name"], $src)) {
        ...
        $stmt = $conn->prepare("INSERT INTO culture (title, content, created_at, Picture) VALUES (?, ?, ?, ?)");
